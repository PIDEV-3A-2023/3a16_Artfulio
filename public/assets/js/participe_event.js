function onClickBtnParticipe(event){
   ...
   const spanCount = this.querySelector('span.js-participes');
    ...
    axios.get(url).then(function (response){
        ...
        spanCount.textContent = response.data.participes;

        //on change d'icone en cas de click
        if(icone.classList.contains('fa-people-pulling')){
         icone.classList.replace('fa-people-pulling','fa-person');
        }else{
            icone.classList.replace('fa-person','fa-people-pulling');
        }
    }).catch(function(error){
        //on entre ici si on est pas connecté
        if(error.response.status === 403) {
            window.alert("vous ne pouvez pas suivre un évenement si vous n'etes pas connecté! ");
        }else{
            //si il y'a un probleme avec le lien
            window.alert("une érreur c'est produite avec le lien! ");
        } 
    })
